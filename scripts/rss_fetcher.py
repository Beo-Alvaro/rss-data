...
def run_poll(url, db_path, interval, once=False):
    logging.info("Starting fetcher: url=%s interval=%s db=%s once=%s", url, interval, db_path, once)
    db = RSSDB(db_path)
    ...
    while True:
        ...
        try:
            ...
            logging.info("Fetched %d items, inserted %d new.", len(items), added)
            ...
            logging.exception("Error fetching/parsing: %s", e)
...
def main():
    ...
    # default to the GMA Regions feed so --url is optional for convenience
    parser.add_argument(
        "--url",
        default="https://data.gmanetwork.com/gno/rss/news/regions/feed.xml",
        help="RSS/Atom feed URL (default: GMA Regions feed)",
    )
    ...
    try:
        run_poll(args.url, args.db, args.interval, once=args.once)
